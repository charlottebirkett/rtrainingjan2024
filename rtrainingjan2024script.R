library(ggplot2)

print(23700187493*524932019740973)
print(34875293475874/294785847)

#*Oh, I just found out adding a * after a # lets you press enter and it carries it to the next line so you can type
#*with fewer interruptions
#*ooh
#*
#*This is much easier
#*
#*
#*Then you just delete it when you need to go back to usual coding
#*
#*
#*
#*
#*

unendingsadness<-seq(from=10, to=1, length=100)
plot(unendingsadness, exp(unendingsadness))

undendingsadness<-seq(10,0, length=100)
timesincelasthotchocolates<-seq(10,0, length=100)
plot(unendingsadness, timesincelasthotchocolates)

myhappiness<-seq(50,2,length=100)
numberofhotchocolates<-seq(50,2,length=100)
plot(myhappiness, numberofhotchocolates)
